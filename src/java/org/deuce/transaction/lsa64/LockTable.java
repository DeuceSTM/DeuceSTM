...
public class LockTable {
	...
	final private static long LOCK = 1L << 63;
	final private static long IDMASK = LOCK - 1;

	// Array 32-bit lock words
	final private static AtomicLongArray locks = new AtomicLongArray(ARRAYSIZE);

	public static long lock(int hash, long id) throws TransactionException {
		assert hash <= MASK;
		while (true) {
	...
	public static long checkLock(int hash, long id) throws TransactionException {
		assert hash <= MASK;
		long lock = locks.get(hash);
		if ((lock & LOCK) != 0) {
			if ((lock & IDMASK) != id) {
