...
/**
 * Context class for TL2 STM with Contention Management support. This class
 * queries its Contention Manager regarding the need for bookkeeping of shared
 * information, like priorities, currently running transaction, etc.
 * 
...
final public class Context implements org.deuce.transaction.Context {
	...
	public final boolean commit() {
		try {
			...
			// Read-only transactions don't have to do anything in order to
			// commit
			if (writeSet.isEmpty()) {
	...
	public final boolean kill(int clockValue) {
		if (clockValue == -1) {
			// The transaction killed itself. No need to CAS - if someone else
			// killed
	...
	/**
	 * Gets the priority of this thread. The priority is calculated based on the
	 * number of locks this thread has opened and not committed on.
	 * 
	...
	private final int lockWriteSet() {
		...
		while (!killedByCM && iter.hasNext() && isStillAlive()) {
			...
			// Attempt to acquire this lock as long as we did not get a command
			// to restart/skip by
			...
			while (!killedByCM && isStillAlive()) {
				...
				if (res[0] == 2) {
					// Lock appeared to be free, but when I tried to CAS it, I
					// failed. The owner of the
					// lock is not known to me, therefore I can't efficiently do
					// contention management, so I just retry.
					continue;
				} else if (res[0] == 1) {
