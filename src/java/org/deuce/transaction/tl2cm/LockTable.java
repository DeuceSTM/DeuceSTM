...
/**
 * Table of locks for accessed locations. Each lock is comprised of a 64-bit
 * number that encodes a version counter, the thread Id of the lock owner and
 * the local clock value of the lock owner. * The lock is unlocked if and only
 * if the owner field equals </code>NO_OWNER</code>.
 * 
 * <p>
 * Based on Guy Korland's and Pascal Felber's work on
 * <code>org.deuce.transaction.tl2.*</code> and
 * <code>org.deuce.transaction.lsa*</code>
 * </p>
 * 
...
public class LockTable {
	...
	public static final int OWNERSIZE = 8; // Allowing for 2^OWNERSIZE threads
											// in the system
	 ...
	 * 
	 * @param hash
	 *            hash code of the location to lock
	 * @param owner
	 *            thread Id of acquiring thread
	 * @param ownerClock
	 *            local clock value of the acquiring thread
	 * @return array of 3 elements. The first element is a status
	 *         indicator:</br> <li>0 if the lock was acquired</li> <li>1 if the
	 *         lock wasn't acquired and the owner is known</li> <li>2 if the
	 *         lock wasn't acquired and the owner is unknown</li> The second
	 *         element is the original lock value (if available) and the third
	 *         element if the new lock value (if available).
	 */
	...
	/**
	 * Attempts to lock the lock even though it might be owned by some other
	 * thread
	 * 
	 * @param hash
	 *            hash code of the location to lock
	 ...
	 *            local clock value of the acquiring thread
	 * @param version
	 *            version of new lock value
	 * @param expectedLock
	 *            the expected lock value
