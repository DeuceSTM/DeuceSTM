...
/**
 * The Kill Priority LockStealer contention manager resolves conflicts exactly
 * the same as Karma LockStelaer. The only difference is that it uses a
 * different priority mechanism; the priority of a transaction is the number of
 * transactions it killed so far. When a transaction kills another transaction
 * the killing transaction increments its priority by 1 + # of transaction the
 * killed transaction killed.
 * 
...
public class KillPrioLS extends AbstractContentionManager {
	...
	public Action resolveReadConflict(ReadFieldAccess readField, Context me, Context other) {
		...
		if (myPrio >= otherPrio) { // If we do > then all priorities will remain
									// 0 since no one will kill
