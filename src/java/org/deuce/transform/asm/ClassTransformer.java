...
public class ClassTransformer extends ByteCodeVisitor implements FieldsHolder {
	...
	public void visitEnd() {
		...
		if (!exclude) {
			...
			if (!visitclinit && fields.size() > 0) { // creates a new <clinit>
	...
	private StaticMethodTransformer createStaticMethodTransformer(MethodVisitor originalMethod,
			MethodVisitor staticMethod) {
		return new StaticMethodTransformer(originalMethod, staticMethod, fields, staticField, className,
	...
	public static Method createNewMethod(String name, String desc) {
		...
		newArguments[newArguments.length - 1] = Context.CONTEXT_TYPE; // add as
																		// a
																		// constant
