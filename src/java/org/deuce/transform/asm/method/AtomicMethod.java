...
/**
 * Used to replaced the original @atomic method with a method that run the
 * transaction loop. On each round the transaction contest reinitialized and the
 * duplicated method is called with the transaction context.
 * 
 * @author Guy Korland
 */
public class AtomicMethod extends MethodAdapter {
	...
	public AtomicMethod(MethodVisitor mv, String className, String methodName, String descriptor, Method newMethod,
			boolean isStatic) {
		super(mv);
	...
	public void visitCode() {
		...
		Label l5 = getContext(contextIndex); // Context context =
		...
		for (int i = 0; i < argumentReolvers.length; ++i) {
			...
			local += argumentReolvers[i].localSize(); // move to the next
														// argument
		}
		...
		if (isStatic)
			mv.visitMethodInsn(INVOKESTATIC, className, methodName, newMethod.getDescriptor()); // ...
																								// =
																								// foo(
																								// ...
		else
			mv.visitMethodInsn(INVOKEVIRTUAL, className, methodName, newMethod.getDescriptor()); // ...
																									// =
																									// foo(
																									// ...
		...
		/*
		 * catch( AbortTransactionException ex) { throw ex; }
		 */
		mv.visitLabel(l25);
		...
		/*
		 * catch( TransactionException ex) { commit = false; }
		 */
		mv.visitLabel(l2);
		mv.visitVarInsn(ASTORE, exceptionIndex);
		Label l13 = new Label();
		...
		if (returnReolver == null) {
			...
			mv.visitVarInsn(returnReolver.loadCode(), resultIndex); // return
																	// result;
	...
	private Label getContext(final int contextIndex) {
		...
		mv.visitLabel(label); // Context context =
								// ContextDelegator.getInstance();
		mv.visitMethodInsn(INVOKESTATIC, ContextDelegator.CONTEXT_DELEGATOR_INTERNAL, "getInstance",
				"()Lorg/deuce/transaction/IContext;");
	...
	private Label throwTransactionException() {
		...
		mv.visitMethodInsn(INVOKESPECIAL, "org/deuce/transaction/TransactionException", "<init>",
				"(Ljava/lang/String;)V");
		mv.visitInsn(ATHROW);
		return label;
	}
