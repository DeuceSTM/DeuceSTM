...
public class MethodTransformer implements MethodVisitor {
	...
	public MethodTransformer(MethodVisitor originalMethod, MethodVisitor copyMethod, String className, int access,
			String methodName, String descriptor, Method newMethod, FieldsHolder fieldsHolder) {

		this.originalMethod = originalMethod;
		this.newMethod = newMethod;
		this.isStatic = (access & ACC_STATIC) != 0;
		this.originalCopyMethod = copyMethod; // save duplicate method without
												// instrumentation.

		// The AnalyzerAdapter delegates the call to the DuplicateMethod, while
		// the DuplicateMethod uses
		...
		AnalyzerAdapter analyzerAdapter = new AnalyzerAdapter(className, access, methodName, descriptor,
				duplicateMethod);
	...
	public void visitCode() {
		...
		if (isIrrevocable) { // Call onIrrevocableAccess
			...
			copyMethod.visitVarInsn(Opcodes.ALOAD, argumentsSize - 1); // load
																		// context
	...
	public AnnotationVisitor visitAnnotation(String desc, boolean visible) {
		...
		return new MethodAnnotationVisitor(originalMethod.visitAnnotation(desc, visible), copyMethod.visitAnnotation(
				desc, visible));
	}
