...
public class DuplicateMethod extends MethodAdapter{
	...
	public void visitFieldInsn(int opcode, String owner, String name, String desc) {
		...
		if (opcode == Opcodes.GETSTATIC || opcode == Opcodes.PUTSTATIC) {
			mv.visitFieldInsn(Opcodes.GETSTATIC, fieldsHolderName, Util.getAddressField(name), ArrayUtil.getTxType(desc).getDescriptor());
		}
		else {
			duplicateTargeObject(opcode, owner, desc);
			mv.visitFieldInsn(Opcodes.GETFIELD, fieldsHolderName, Util.getAddressField(name), ArrayUtil.getTxType(desc).getDescriptor());
		}
...
//		/* opt */mv.visitInsn(Opcodes.POP);
		...
		case Opcodes.GETFIELD:  //	ALOAD 0: this (stack status)
			...
			super.visitFieldInsn( Opcodes.GETFIELD, fieldsHolderName, Util.getAddressField(name) , ArrayUtil.getTxType(desc).getDescriptor());
			super.visitVarInsn(Opcodes.ALOAD, argumentsSize - 1); // load context
		...
		case Opcodes.GETSTATIC: // check support for static fields
			...
			super.visitFieldInsn(opcode, owner, name, desc);
			super.visitFieldInsn(Opcodes.GETSTATIC, fieldsHolderName, Util.getAddressField(name) , ArrayUtil.getTxType(desc).getDescriptor());
			super.visitVarInsn(Opcodes.ALOAD, argumentsSize - 1); // load context
			super.visitMethodInsn( Opcodes.INVOKESTATIC, ContextDelegator.CONTEXT_DELEGATOR_INTERNAL,
					ContextDelegator.READ_METHOD_NAME, ContextDelegator.getReadMethodDesc(type));
	...
	private void addBeforeReadCall(String owner, String name, String desc, boolean isStatic) {
		if (!isStatic) {
			...
			super.visitFieldInsn( Opcodes.GETFIELD, owner, Util.getAddressField(name) , ArrayUtil.getTxType(desc).getDescriptor());
		}
		else {
			super.visitFieldInsn( Opcodes.GETSTATIC, owner, Util.getAddressField(name) , ArrayUtil.getTxType(desc).getDescriptor());
		}
//		/* opt */super.visitInsn(Opcodes.DUP);
	...
	public void visitInsn(int opcode) {
		...
		case Opcodes.AALOAD:
			...
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_OBJ_DESC_META : ContextDelegator.READ_ARRAY_METHOD_OBJ_DESC;
			load = true;
			break;
		case Opcodes.BALOAD:
			type = Type.BYTE_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_BYTE_DESC_META : ContextDelegator.READ_ARRAY_METHOD_BYTE_DESC;
			load = true;
			break;
		case Opcodes.CALOAD:
			type = Type.CHAR_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_CHAR_DESC_META : ContextDelegator.READ_ARRAY_METHOD_CHAR_DESC;
			load = true;
			break;
		case Opcodes.SALOAD:
			type = Type.SHORT_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_SHORT_DESC_META : ContextDelegator.READ_ARRAY_METHOD_SHORT_DESC;
			load = true;
			break;
		case Opcodes.IALOAD:
			type = Type.INT_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_INT_DESC_META : ContextDelegator.READ_ARRAY_METHOD_INT_DESC;
			load = true;
			break;
		case Opcodes.LALOAD:
			type = Type.LONG_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_LONG_DESC_META : ContextDelegator.READ_ARRAY_METHOD_LONG_DESC;
			load = true;
			break;
		case Opcodes.FALOAD:
			type = Type.FLOAT_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_FLOAT_DESC_META : ContextDelegator.READ_ARRAY_METHOD_FLOAT_DESC;
			load = true;
			break;
		case Opcodes.DALOAD:
			type = Type.DOUBLE_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.READ_ARRAY_METHOD_DOUBLE_DESC_META : ContextDelegator.READ_ARRAY_METHOD_DOUBLE_DESC;
			load = true;
			break;
			
		case Opcodes.AASTORE:
			type = Type.getType(Object.class);
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_OBJ_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_OBJ_DESC;
			store = true;
			break;
		case Opcodes.BASTORE:
			type = Type.BYTE_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_BYTE_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_BYTE_DESC;
			store = true;
			break;
		case Opcodes.CASTORE:
			type = Type.CHAR_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_CHAR_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_CHAR_DESC;
			store = true;
			break;
		case Opcodes.SASTORE:
			type = Type.SHORT_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_SHORT_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_SHORT_DESC;
			store = true;
			break;
		case Opcodes.IASTORE:
			type = Type.INT_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_INT_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_INT_DESC;
			store = true;
			break;
		case Opcodes.LASTORE:
			type = Type.LONG_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_LONG_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_LONG_DESC;
			store = true;
			break;
		case Opcodes.FASTORE:
			type = Type.FLOAT_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_FLOAT_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_FLOAT_DESC;
			store = true;
			break;
		case Opcodes.DASTORE:
			type = Type.DOUBLE_TYPE;
			desc = ContextDelegator.inLocalMetadata() ? ContextDelegator.WRITE_ARRAY_METHOD_DOUBLE_DESC_META : ContextDelegator.WRITE_ARRAY_METHOD_DOUBLE_DESC;
			store = true;
			break;
		}
...
//				super.visitTypeInsn(Opcodes.CHECKCAST, (String)analyzerAdapter.stack.get(analyzerAdapter.stack.size()-1));
				String tyDesc = getAnalyzerTopStackObjectDesc();
				Type tt = ArrayUtil.getTxArrayArrayType(tyDesc); 
//				System.out.println("Type="+type+" TyDesc="+tyDesc+" TT="+tt);
				if (tyDesc.lastIndexOf("[") != -1) {
//					String arr = tyDesc.substring(0, tyDesc.lastIndexOf("[")+1);
//					super.visitTypeInsn(Opcodes.CHECKCAST, arr+ArrayUtil.getTxArrayType(tyDesc).getDescriptor());
