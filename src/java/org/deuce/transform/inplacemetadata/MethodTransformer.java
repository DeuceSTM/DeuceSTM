...
{
  final static private String UNSAFE_DESCRIPTOR = 
      Type.getDescriptor(Unsafe.class);
      ...
      FieldsHolder  fieldsHolder)
  {
    ...
    // The AnalyzerAdapter delegates the call to the DuplicateMethod, while the
    // DuplicateMethod uses the analyzer for stack state in the original method.
    ...
    if (originalCopyMethod != null) {
      duplicateMethod = new DuplicateMethod(copyMethod, isStatic, newMethod,
          fieldsHolder);
  ...
  public AnnotationVisitor visitAnnotation(String desc, boolean visible) {
    // FIXME we might saw other annotations before and we need to put it on the
    // new AtomicMethod need to create an atomic method from the original method
    ...
    if (!desc.contains("org/junit")) {
      return new MethodAnnotationVisitor(originalMethod.visitAnnotation(desc,
          visible), originalCopyMethod != null ? copyMethod.visitAnnotation(
          desc, visible) : null);
    } else {
  ...
  private String getAnalyzerTopStackObjectDesc() {
    ...
    if (analyzerAdapter.stack.get(analyzerAdapter.stack.size() - 1) 
          instanceof Integer)
    {
      return Type.INT_TYPE.getDescriptor();
    } else {
      tos = (String) analyzerAdapter.stack
  ...
  public void visitInsn(int opcode) {
    if (!ignore) {
      ...
      /*
       * If the opcode is one of the ALOAD family, that means the stack is of
       * the form: 
       * 
       * [ ..., arrayref, index => 
       * 
       * We must replace the instruction with an AALOAD, because the array 
       * elements are TxArrField. Then we cast it to the appropriate TxArrField
       * type (correspondent to the original _ALOAD specific type instruction), 
       * and load TxArrField.array[TxArrField.index]. 
       * 
       * Legend:
       *  arr    -> arrayref 
       *  idx    -> array access index (int) 
       *  obj    -> objectref 
       *  obj*   -> objectref (correct TxArrField type)
       *  val    -> value of the original array's element 
          ...
          || opcode == Opcodes.FALOAD || opcode == Opcodes.DALOAD)
      {
        switch (opcode) {
          case Opcodes.AALOAD:
            if (Type.getType(getAnalyzerTopStackObjectDesc()).getSort()
                  == Type.ARRAY) {
            ...
            == Type.ARRAY)
        {
          originalMethod.visitTypeInsn(Opcodes.CHECKCAST, 
              ArrayUtil.getTxArrayArrayType(t).getInternalName());
        } else {
        ...
        if (opcode == Opcodes.AALOAD) {
           ...
           * objectref on top of this stack is a TxArrObjectField. It also means
           * it is being used in a multiarray context. Therefore we must get the
           * nextDim field reference instead of loading 
           * TxArrField.array[TxArrField.index].
           */
          if (ArrayUtil.getArrayDim(arrayDesc) > 1) {
            originalMethod.visitFieldInsn(Opcodes.GETFIELD,
                t.getInternalName(), "nextDim", "Ljava/lang/Object;");
          } else {
            originalMethod.visitInsn(Opcodes.DUP);
            // ..., obj*, obj* =>
            originalMethod.visitFieldInsn(Opcodes.GETFIELD,
                t.getInternalName(), "array",
                "[" + Type.getDescriptor(Object.class));
            // ..., obj*, arr =>
            originalMethod.visitInsn(Opcodes.SWAP);
            // ..., arr, obj* =>
            originalMethod.visitFieldInsn(Opcodes.GETFIELD,
                t.getInternalName(), "index", Type.getDescriptor(int.class));
            // ..., arr, idx =>
          ...
          originalMethod.visitInsn(Opcodes.DUP);
          // ..., obj*, obj* =>
          originalMethod.visitFieldInsn(Opcodes.GETFIELD, t.getInternalName(),
              "array", "[" + ArrayUtil.getTypeFromTxArray(t).getDescriptor());
          // ..., obj*, arr =>
          originalMethod.visitInsn(Opcodes.SWAP);
          // ..., arr, obj* =>
          originalMethod.visitFieldInsn(Opcodes.GETFIELD, t.getInternalName(),
              "index", Type.getDescriptor(int.class));
          // ..., arr, idx =>
          originalMethod.visitInsn(opcode);
        }
        // ..., val =>
        if (opcode == Opcodes.AALOAD) {
          ...
          {
            originalMethod.visitTypeInsn(Opcodes.CHECKCAST, 
                ArrayUtil.getTxArrayArrayType(
                    getAnalyzerTopStackObjectDesc()).getInternalName());
          } else {
            originalMethod
        ...
        /*
         * If the opcode is one of the ASTORE family, that means the stack is of
         * the form: 
         * 
         * [ ..., arrayref, index, value => 
         * 
         * We must AALOAD the arrayref[index] element first, cast it to the 
         * appropriate TxArrField type, and substitute the arrayref and index 
         * below with the TxArrField.array and TxArrField.index, respectively.
         * 
         * Legend: 
         *  arr    -> arrayref 
         *  idx    -> array access index (int) 
          ...
          || opcode == Opcodes.FASTORE || opcode == Opcodes.DASTORE)
      {
        ...
        /*
         * If the original arrayref was multidimensional, that means the
         * objectref on this stack is a TxArrObjectField. It also means it is
         * being used in a multiarray context. Therefore we must update the
         * nextDim field reference instead of 
         * TxArrField.array[TxArrField.index].
         */
        if (opcode == Opcodes.AASTORE && ArrayUtil.getArrayDim(arrayDesc) > 1) {
          ...
          // ..., obj*, newval =>
          originalMethod.visitFieldInsn(Opcodes.PUTFIELD,
              arrayType.getInternalName(), "nextDim", "Ljava/lang/Object;");
        } else {
          originalMethod.visitInsn(Opcodes.DUP);
          // ..., newval, obj*, obj* =>
          originalMethod.visitFieldInsn(Opcodes.GETFIELD,
              arrayType.getInternalName(), "array", "[" + t.getDescriptor());
          // ..., newval, obj*, arr =>
          originalMethod.visitInsn(Opcodes.SWAP);
          // ..., newval, arr, obj* =>
          originalMethod.visitFieldInsn(Opcodes.GETFIELD,
              arrayType.getInternalName(), "index",
  ...
  public void visitIntInsn(int opcode, int operand) {
    ...
    if (!ignore) {
       ...
       * [ ..., arrayref => 
       * 
       * So, the desired transformation is to start with the original arrayref 
       * on top of the stack, and finish with the new TxArray arrayref instead. 
       * 
       * [ ..., arrayref (TxArray) => 
       * 
       * In the end, the original array is "lost" in the heap, and the only 
       * objects holding a reference to it will be the TxArray elements. 
       * 
       * Legend:
       *  arr  -> arrayref
       *  obj  -> objectref 
       *  len  -> result of ARRAYLENGTH (int) 
       *  arr* -> arrayref (TxArray)
       */
      if (opcode == Opcodes.NEWARRAY) {
        ...
        // ..., arr, len =>
        originalMethod.visitTypeInsn(Opcodes.ANEWARRAY,
            arrayType.getInternalName());
        // ..., arr, arr* =>
        ...
        originalMethod.visitMethodInsn(Opcodes.INVOKESPECIAL,
            arrayType.getInternalName(), "<init>",
      ...
      int    index)
  {
    ...
    if (originalCopyMethod != null) {
      copyMethod.visitLocalVariable(name, desc, signature, getLabel(start),
          getLabel(end), index);
    }
  }
      ...
      String desc)
  {
    if (!ignore
      ...
      if (isBC && hasArrayReturnOrArgument(desc)) {
        ...
        NonInsnMethod nim = clazzT.new NonInsnMethod(name, nimDesc, desc,
            owner, opcode);
        ...
        originalMethod.visitMethodInsn(Opcodes.INVOKESTATIC, nim.insnOwner,
            nim.insnName, nim.insnDesc);
        ...
        if (originalCopyMethod != null) {
          ...
          copyMethod.visitMethodInsn(Opcodes.INVOKESTATIC, nim.insnOwner,
              nim.insnName, nim.insnDesc);
        ...
        originalMethod.visitMethodInsn(opcode, owner, name, ArrayUtil
            .updateMethodArrayArgumentsAndReturn(desc).getDescriptor());
        if (originalCopyMethod != null) {
  ...
  public void visitMultiANewArrayInsn(String desc, int dims) {
    ...
    if (!ignore) {
      String name = ArrayUtil.getTxArrayType(desc).getInternalName().replace(
          "/", "_") + "_multiarray";
      ...
      MultiArrayMethod mim = clazzT.new MultiArrayMethod(name, new Method(name,
          returnType, args).getDescriptor(), dims, totalDims, 
          Type.getType(desc));
      ...
      originalMethod.visitInsn(Opcodes.ACONST_NULL);
      // ..., arr, null =>
      originalMethod.visitMethodInsn(Opcodes.INVOKESTATIC, className, mim.name,
          mim.desc);
      // ... arr* =>
      ...
      if (originalCopyMethod != null) {
        ...
        copyMethod.visitInsn(Opcodes.ACONST_NULL);
        // ..., arr, null =>
        copyMethod.visitMethodInsn(Opcodes.INVOKESTATIC, className, mim.name,
            mim.desc);
        // ..., arr* =>
  ...
  public AnnotationVisitor visitParameterAnnotation(int parameter, String desc,
      boolean visible) {
    return new MethodAnnotationVisitor(originalMethod.visitParameterAnnotation(
        parameter, desc, visible), copyMethod.visitParameterAnnotation(
    ...
    if (!ignore) {
      ...
      if (opcode == Opcodes.ANEWARRAY) {
        ...
        if (type.lastIndexOf("[") == -1) {
          ...
          ctorSignature = "([" + Type.getDescriptor(Object.class) 
              + Type.getDescriptor(int.class) + ")" 
        ...
        Type arrayType = ArrayUtil.getArrayType(
            ArrayUtil.getTxArrayArrayType(type));
        ...
        originalMethod.visitMethodInsn(
            Opcodes.INVOKESPECIAL,
            arrayType.getInternalName(),
            "<init>",
            ctorSignature);
