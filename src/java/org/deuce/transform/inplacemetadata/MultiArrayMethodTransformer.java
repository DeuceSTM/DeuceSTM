...
public class MultiArrayMethodTransformer extends MethodAdapter {
   ...
   * The local variables of this method, regarding the example, are as follows:
      ...
      Type          originalType)
  {
    ...
    Type arrayType = currDim < totalDims ? 
        ArrayUtil.getTxArrayType(Type.getDescriptor(Object.class)) 
        : ArrayUtil.getTxArrayType(originalType.getDescriptor());

    String ctorParamDesc = null;
    if (currDim < totalDims)
      // ([Ljava/lang/Object;ILjava/lang/Object;)V
      ctorParamDesc = "([" + Type.getDescriptor(Object.class) 
      ...
      // etc...
      ctorParamDesc = "([" 
          + ArrayUtil.getArrayFromTxArray(arrayType).getDescriptor() 
    ...
    if (currDim < nDims) {
      // ... =>
      initializeMatrixDimension(mv, currDim + 1, nDims, totalDims, 
          originalType);
      // ... =>
      mv.visitVarInsn(Opcodes.ALOAD, arrM_idx);
      // ..., arr* =>
