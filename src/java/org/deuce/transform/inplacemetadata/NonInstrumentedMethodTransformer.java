...
public class NonInstrumentedMethodTransformer extends MethodAdapter {
	...
	public NonInstrumentedMethodTransformer(MethodVisitor mv, ClassTransformer classTransformer) {
		super(mv);
	...
	public void visitCode() {
		...
		if (insnM.opcode != Opcodes.INVOKESTATIC && insnM.opcode != Opcodes.INVOKESPECIAL) {
			super.visitVarInsn(Opcodes.ALOAD, 0);
		}

		int i = (insnM.opcode != Opcodes.INVOKESTATIC && insnM.opcode != Opcodes.INVOKESPECIAL ? 1 : 0);
		...
		super.visitMethodInsn(insnM.opcode, insnM.owner, insnM.name, insnM.origDesc);
	...
	private void multiArrayToTxArray(Type originalType, MethodVisitor mv) {
		...
		MultiArrayMethod mim = clazzT.new MultiArrayMethod(name, new Method(name, returnType, args).getDescriptor(),
		...
		// ..., arr, null =>
		mv.visitMethodInsn(Opcodes.INVOKESTATIC, insnM.insnOwner, mim.name, mim.desc);
		// ... arr* =>
	}
}
