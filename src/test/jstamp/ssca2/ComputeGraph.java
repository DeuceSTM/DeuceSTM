...
public class ComputeGraph {
		...
		for (int i = lss.i_start; i < lss.i_stop; i++) {
			...
			for (int j = GPtr.outVertexIndex[i]; j < jend; j++) {
				...
				if (k == kend) {
					...
					atomicMethodThree(GPtr, MAX_CLUSTER_SIZE, impliedEdgeList, auxArr, i, v);
	...
	@Atomic
	private static void atomicMethodThree(Graph GPtr, int MAX_CLUSTER_SIZE, int[][] impliedEdgeList, int[][] auxArr,
			int i, int v) {
	...
	@Atomic
	private static void atomicMethodTwo(ComputeGraph computeGraphArgs, int outVertexListSize) {
		computeGraphArgs.global_outVertexListSize = computeGraphArgs.global_outVertexListSize + outVertexListSize;
	}

	@Atomic
	private static void atomicMethodOne(ComputeGraph computeGraphArgs, int maxNumVertices) {
