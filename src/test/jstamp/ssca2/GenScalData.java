...
public class GenScalData {
	...
	public static void genScalData_seq(Globals glb, GraphSDG SDGdataPtr, GenScalData gsd, Alg_Radix_Smp radixsort) {
		...
		/*
		 * Allocate mem for Clique array Estimate number of clique required and
		 * pad by 50%
		 */
		...
		/*
		 * TODO: if required #ifdef WRITE_RESULT_FILES // Write the generated
		 * cliques to file for comparison with Kernel 4 FILE* outfp =
		 * fopen("cliques.txt", "w"); fSystem.out.println(outfp,
		 * "No. of cliques - %lu\n", totCliques); for (i = 0; i < totCliques;
		 * i++) { fSystem.out.println(outfp, "Clq %lu - ", i); long j; for (j =
		 * firstVsInCliques[i]; j <= lastVsInCliques[i]; j++) {
		...
		/*
		 * Tmp array to keep track of the no. of parallel edges in each
		 * direction
		 */
		...
		for (int i = 0; i < numEdgesPlaced; i++) {
			if (SDGdataPtr.intWeight[i] <= 0) {
				for (int j = 0; j < glb.MAX_STRLEN; j++) {
					SDGdataPtr.strWeight[(-SDGdataPtr.intWeight[i]) * glb.MAX_STRLEN + j] = (byte) (1 + randomPtr
							.posrandom_generate() % 127);
				}
			}
		}
		...
		radixsort.all_radixsort_node_aux_s3_seq(numEdgesPlaced, startVertex, SDGdataPtr.startVertex, endVertex,
	...
	public static void genScalData(int myId, int numThread, Globals glb, GraphSDG SDGdataPtr, GenScalData gsd,
		...
		if (myId == 0) {
			...
			/*
			 * Sum up vertices in each clique to determine the lastVsInCliques
			 * array
			 */
		 ...
		 * TODO : if required
		 * 
		 * #ifdef WRITE_RESULT_FILES Barrier.enterBarrier();
		 * 
		 * // Write the generated cliques to file for comparison with Kernel 4
		 * if (myId == 0) { FILE* outfp = fopen("cliques.txt", "w");
		 * fSystem.out.println(outfp, "No. of cliques - %lu\n", totCliques); for
		 * (i = 0; i < totCliques; i++) { fSystem.out.println(outfp,
		 * "Clq %lu - ", i); int j; for (j = firstVsInCliques[i]; j <=
		 * lastVsInCliques[i]; j++) { fSystem.out.println(outfp, "%lu ",
		 * permV[j]); } fSystem.out.println(outfp, "\n"); } fclose(outfp); }
		 * 
		 * Barrier.enterBarrier(); #endif
		 */
		...
		Alg_Radix_Smp.all_radixsort_node_aux_s3(myId, numThread, numEdgesPlaced, startVertex, SDGdataPtr.startVertex,
				endVertex, SDGdataPtr.endVertex, radixsort);
