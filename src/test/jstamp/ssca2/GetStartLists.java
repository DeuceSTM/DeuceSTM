...
public class GetStartLists {
	...
	public static void getStartLists(int myId, int numThread, Globals glb, GetStartLists gsl) {
		...
		gsl.maxIntWtListSize += i_edgeCounter; // FIXME should be inside an
												// atomic block
		...
		for (i = lss.i_start; i < lss.i_stop; i++) {
			...
			/*
			 * if (strncmp(gsl.GPtr.strWeight+i*glb.MAX_STRLEN, SOUGHT_STRING,
			 * glb.MAX_STRLEN) == 0) { // // Find the corresponding endVertex //
			 * 
			 * int t; for (t = 0; t < gsl.GPtr.numEdges; t++) { if
			 * (gsl.GPtr.intWeight[t] == -i) { break; } }
			 * 
			 * int j; for (j = 0; j < gsl.GPtr.numDirectedEdges; j++) { if
			 * (gsl.GPtr.paralEdgeIndex[j] > t) { break; } }
			 * tmpEdgeList[i_edgeCounter].endVertex =
			 * gsl.GPtr.outVertexList[j-1]; tmpEdgeList[i_edgeCounter].edgeNum =
			 * j-1;
			 * 
			 * for (t = 0; t < gsl.GPtr.numVertices; t++) { if
			 * (gsl.GPtr.outVertexIndex[t] > j-1) { break; } }
			 * tmpEdgeList[i_edgeCounter].startVertex = t-1; i_edgeCounter++; }
			 */
