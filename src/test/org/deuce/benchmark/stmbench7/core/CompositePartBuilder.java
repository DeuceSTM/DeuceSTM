...
/**
 * Used to create and destroy composite parts while maintaining the consistency
 * of the data structure and the indexes.
 */
@Immutable
public class CompositePartBuilder extends DesignObjBuilder {
	...
	public CompositePartBuilder(Index<Integer, CompositePart> compositePartIdIndex,
			Index<String, Document> documentTitleIndex, Index<Integer, AtomicPart> partIdIndex,
	...
	/**
	 * Creates a component (composite part) with the documentation and a graph
	 * of atomic parts and updates all the relevant indexes.
	 */
	...
	private void createConnections(AtomicPart[] parts) {
		...
		for (int partNum = 0; partNum < Parameters.NumAtomicPerComp; partNum++) {
			...
			parts[partNum].connectTo(parts[connectTo], createType(), ThreadRandom.nextInt(Parameters.XYRange) + 1);
		}
