...
/**
 * Test of invariants of indexes, and their consistency with the main data
 * structure. Checks also invariants of the composite parts that are not
 * reachable from base assemblies.
 */
@Immutable
@ThreadLocal
public class IndexTest extends InvariantTest {
	...
	public static void checkInvariants(Setup setup, boolean initial, TraversedObjects traversedObjects) {
		...
		for (AtomicPart traversedPart : traversedObjects.atomicParts) {
			int id = traversedPart.getId();
			LargeSet<AtomicPart> sameBuildDateParts = atomicPartBuildDateIndex.get(traversedPart.getBuildDate());
	...
	private static void checkIndexValue(String index, Object traversedObject, Object indexedObject, int elementId) {
		if (indexedObject == null)
			reportError(index, "element with id = " + elementId + " in the data structure but not in the index");
