...
/**
 * Replays sequentially a concurrent execution. Used to check whether a given
 * concurrent execution ensures opacity, i.e., whether the synchronization
 * method used in the execution was correctly synchronizing threads during this
 * execution.
 */
@NonAtomic
public class SequentialReplayThread extends BenchThread {

	public SequentialReplayThread(Setup setup, double[] operationCDF, ArrayList<ReplayLogEntry> replayLog) {
	...
	public void run() {
		...
		for (ReplayLogEntry entry : replayLog) {
			...
			// System.out.println(entry.threadNum + " -- " +
			...
			if (result != entry.result || failed != entry.failed) {
				...
				throw new RuntimeError("Different operation result in the sequential execution (" + "operation "
						+ opName + "): " + "Sequential: result = " + result + ", failed = " + failed + ". "
