package org.deuce.benchmark.stmbench7.correctness.opacity;
...
import org.deuce.benchmark.stmbench7.core.DesignObj;
...
/**
 * An operation that compares two benchmark data structures and throws a
 * RuntimeError if they differ. Used to check whether a concurrent execution
 * ensures opacity.
 */
@Immutable
@ThreadLocal
public class StructureComparisonOperation implements Operation {
	...
	private void checkAssembliesEqual(Assembly assembly1, Assembly assembly2) {
		...
		checkReferences(assembly1.getModule(), assembly2.getModule(), assembly1, id1, assembly2, id2, "module");
		checkReferences(assembly1.getSuperAssembly(), assembly2.getSuperAssembly(), assembly1, id1, assembly2, id2,
				"superAssembly");
	...
	private void checkComplexAssembliesEqual(ComplexAssembly assembly1, ComplexAssembly assembly2) {
		int id1 = assembly1.getId(), id2 = assembly2.getId();
		checkEqual(assembly1.getLevel(), assembly2.getLevel(), assembly1, id1, assembly2, id2, "level");

		ImmutableCollection<Assembly> subAss1 = assembly1.getSubAssemblies(), subAss2 = assembly2.getSubAssemblies();
		checkEqual(subAss1.size(), subAss2.size(), assembly1, id1, assembly2, id2, "subAssemblies.size()");
	...
	private void checkBaseAssembliesEqual(BaseAssembly assembly1, BaseAssembly assembly2) {
		int id1 = assembly1.getId(), id2 = assembly2.getId();
		ImmutableCollection<CompositePart> comp1 = assembly1.getComponents(), comp2 = assembly2.getComponents();
		checkEqual(comp1.size(), comp2.size(), assembly1, id1, assembly2, id2, "components.size()");

		Iterator<CompositePart> it1 = comp1.iterator(), it2 = comp2.iterator();
		while (it1.hasNext()) {
			CompositePart component1 = it1.next(), component2 = it2.next();
			checkReferences(component1, component2, assembly1, id1, assembly2, id2, "components");
			checkEqual(component1.getId(), component2.getId(), assembly1, id1, assembly2, id2, "components");
		}
	}

	private void checkDesignLibrariesEqual() {
		Index<Integer, CompositePart> library1 = setup1.getCompositePartIdIndex(), library2 = setup2
				.getCompositePartIdIndex();
	...
	private void checkCompositePartsEqual(CompositePart component1, CompositePart component2) {
		...
		ImmutableCollection<BaseAssembly> usedIn1 = component1.getUsedIn(), usedIn2 = component2.getUsedIn();
		checkEqual(usedIn1.size(), usedIn2.size(), component1, id1, component2, id2, "usedIn.size()");

		Iterator<BaseAssembly> it1 = usedIn1.iterator(), it2 = usedIn2.iterator();
		while (it1.hasNext()) {
			BaseAssembly assembly1 = it1.next(), assembly2 = it2.next();
			checkReferences(assembly1, assembly2, component1, id1, component2, id2, "usedIn");
			checkEqual(assembly1.getId(), assembly2.getId(), component1, id1, component2, id2, "usedIn");
		}
		...
		LargeSet<AtomicPart> parts1 = component1.getParts(), parts2 = component2.getParts();
		checkReferences(parts1, parts2, component1, id1, component2, id2, "parts");
		checkEqual(parts1.size(), parts2.size(), component1, id1, component2, id2, "parts.size()");
	...
	private void checkDocumentsEqual(Document doc1, Document doc2) {
		...
		checkReferences(doc1.getCompositePart(), doc2.getCompositePart(), doc1, id1, doc2, id2, "part");
	}
	...
	private void checkAtomicPartsEqual(AtomicPart part1, AtomicPart part2) {
		...
		checkReferences(part1.getPartOf(), part2.getPartOf(), part1, id1, part2, id2, "partOf");

		ImmutableCollection<Connection> to1 = part1.getToConnections(), to2 = part2.getToConnections();
	...
	private void checkIndexesEqual() {
		Index<Integer, AtomicPart> atomicPartIdIndex1 = setup1.getAtomicPartIdIndex(), atomicPartIdIndex2 = setup2
				.getAtomicPartIdIndex();
		checkEqual(atomicPartIdIndex1, atomicPartIdIndex2, "AtomicPart.id");

		Index<Integer, LargeSet<AtomicPart>> atomicPartBuildDateIndex1 = setup1.getAtomicPartBuildDateIndex(), atomicPartBuildDateIndex2 = setup2
				.getAtomicPartBuildDateIndex();
		checkEqual(atomicPartBuildDateIndex1, atomicPartBuildDateIndex2, "AtomicPart.buildDate");

		Index<String, Document> documentTitleIndex1 = setup1.getDocumentTitleIndex(), documentTitleIndex2 = setup2
				.getDocumentTitleIndex();
		checkEqual(documentTitleIndex1, documentTitleIndex2, "Document.title");

		Index<Integer, CompositePart> compositePartIdIndex1 = setup1.getCompositePartIdIndex(), compositePartIdIndex2 = setup2
				.getCompositePartIdIndex();
		checkEqual(compositePartIdIndex1, compositePartIdIndex2, "CompositePart.id");

		Index<Integer, BaseAssembly> baseAssemblyIdIndex1 = setup1.getBaseAssemblyIdIndex(), baseAssemblyIdIndex2 = setup2
				.getBaseAssemblyIdIndex();
		checkEqual(baseAssemblyIdIndex1, baseAssemblyIdIndex2, "BaseAssembly.id");

		Index<Integer, ComplexAssembly> complexAssemblyIdIndex1 = setup1.getComplexAssemblyIdIndex(), complexAssemblyIdIndex2 = setup2
				.getComplexAssemblyIdIndex();
	...
	private void checkEqual(Iterable setIter1, Iterable setIter2, String setName) {
		...
		for (Object obj : setIter1)
			set1.add(obj);
		for (Object obj : setIter2)
			set2.add(obj);
	...
	private void reportError(Object obj1, int id1, Object obj2, int id2, String message) {
		throw new RuntimeError("Object " + obj1.toString() + " with id = " + id1 + " != object " + obj2.toString()
				+ " with id = " + id2 + ": " + message);
	}

	private void checkEqual(int val1, int val2, Object obj1, int id1, Object obj2, int id2, String attribute) {
		if (val1 != val2)
			reportError(obj1, id1, obj2, id2, "attribute " + attribute + " --> " + val1 + " != " + val2);
	}

	private void checkEqual(String val1, String val2, Object obj1, int id1, Object obj2, int id2, String attribute) {
		if (!val1.equals(val2))
			reportError(obj1, id1, obj2, id2, "attribute " + attribute + " --> " + val1 + " != " + val2);
	}

	private void checkReferences(Object val1, Object val2, Object obj1, int id1, Object obj2, int id2, String attribute) {

		if (val1 != null && val1 == val2)
			reportError(obj1, id1, obj2, id2, "attribute " + attribute + " --> " + "references equal");
