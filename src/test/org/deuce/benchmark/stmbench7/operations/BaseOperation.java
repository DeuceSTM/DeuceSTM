...
public abstract class BaseOperation implements Operation {
	 ...
	 * later reuse.
	 * 
	 * At first glance, it may seem more complicated than it is necessary, but
	 * that is because we want to use only locking implemented by an Index and
	 * Set implementations.
	 */
	public static void addAtomicPartToBuildDateIndex(Index<Integer, LargeSet<AtomicPart>> atomicPartBuildDateIndex,
			AtomicPart atomicPart) {
		LargeSet<AtomicPart> newSet = BackendFactory.instance.<AtomicPart> createLargeSet();
		newSet.add(atomicPart);
		LargeSet<AtomicPart> sameDateSet = atomicPartBuildDateIndex.putIfAbsent(atomicPart.getBuildDate(), newSet);
		if (sameDateSet != null)
			sameDateSet.add(atomicPart);
	}

	/**
	 * The method of removing and AtomicPart from the AtomicPartBuildDateIndex
	 * is non-trivial and is used in a few places. That is why it is put here
	 * for later reuse.
	 */
	public static void removeAtomicPartFromBuildDateIndex(
			Index<Integer, LargeSet<AtomicPart>> atomicPartBuildDateIndex, AtomicPart atomicPart) {
