...
public class QueueTest {
	...
	public static void main(String[] argv) {
		...
		System.out.format("Finished after %d miliseconds. Success ratio %f", new Long(
				(System.nanoTime() - start) / 1000000), successRate);
	}
	...
	private static class MyThread extends Thread {
		...
		public void run() {
			...
			for (int i = 0; i < NUMBER_OF_ATTEMPTS; ++i) {
				...
				try {
					// we try to insert anyway to try to prevent queue from
					// emptying
					queue.insert(value + ADD_FACTOR);
					// System.out.println(myId + " " + (value + ADD_FACTOR) +
					// " inserted ");
				} catch (IllegalStateException e) {
					System.out.println(e);
					failureAttempt = true;
